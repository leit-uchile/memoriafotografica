...
const FilterModal = (props) => {
    ...
    const toggle = () => {setNewphoto(photo); setLoading(false); setModal(!modal)};
    ...
    const saveChanges = () => {
      ...
      let e = editPhoto(to_send.id, to_send).then(response => {
        setLoading(!loading)
    ...
    return (
        <div>
          ...
          <Modal isOpen={modal} toggle={toggle} className={className}>
            <ModalHeader toggle={toggle}>Curando fotografía: {photo.title} </ModalHeader>
            <ModalBody>    
              <Form>
                ...
                <Row form>
                  <FormGroup check>
                  <Label check>
                    <Input type="checkbox" name="approved" checked={newphoto.approved} onChange={handleCheckboxChange}/>{' '}
                    Foto Aprobada
                  </Label>
                  </FormGroup>
                  <FormGroup check>
                  <Label check>
                    <Input type="checkbox" name="censure" checked={newphoto.censure} onChange={handleCheckboxChange}/>{' '}
                    Foto Censurada
                  </Label>
                  </FormGroup>
                </Row>
            ...
            <ModalFooter>
              <Button color="primary" onClick={saveChanges}>{!loading ? "Guardar Cambios" : "enviando..."}</Button>{' '}           
